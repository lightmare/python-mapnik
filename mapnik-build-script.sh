...
MASON_NAME=${BASH_SOURCE%/*/script.sh}
MASON_NAME=${MASON_NAME##*/}
MASON_VERSION=${BASH_SOURCE%/script.sh}
MASON_VERSION=${MASON_VERSION##*/}
MASON_LIB_FILE=lib/libmapnik.${MASON_DYNLIB_SUFFIX}

# saving CC/CXX here in case mason.sh changes them won't help,
# because for some reason mason.sh is sourced by the calling mason
# executable before this script is called, and so these variables
# may already be overwritten

. ${MASON_DIR}/mason.sh

# force CC/CXX values from the outside

if test -n "${FORCE_CC+isset}" && ! test "$FORCE_CC" = "${CC-}"; then
    echo "forcing CC=$FORCE_CC (was ${CC-unset})"
    CC=$FORCE_CC
    which -a -- $FORCE_CC
fi >&2

if test -n "${FORCE_CXX+isset}" && ! test "$FORCE_CXX" = "${CXX-}"; then
    echo "forcing CXX=$FORCE_CXX (was ${CXX-unset})"
    CXX=$FORCE_CXX
    which -a -- $FORCE_CXX
fi >&2
...
function mason_load_source {
    ...
    if ! test -d ${MASON_BUILD_PATH}; then
        git clone --depth 1 --branch "${MASON_VERSION}" https://github.com/mapnik/mapnik ${MASON_BUILD_PATH}
        git -C ${MASON_BUILD_PATH} submodule update --init deps/
    fi
}
...
function mason_compile {
    ...
    perl -i.bak -pwe '
        print ": \${MASON_ROOT:=./mason_packages}\n" if $. == 2;
        s[ \s -isysroot \s \S+  ][]xg;
        s[ \Q$ENV{PWD}\E        ][.]xg;
        s[ \Q$ENV{MASON_ROOT}\E ][\${MASON_ROOT}]xg;
        s[ ^CONFIG_\w*= \K \047 (\$.*) \047 ]["$1"]x;
        ' "$MASON_PREFIX/bin/mapnik-config"
