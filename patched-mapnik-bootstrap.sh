...
function install() {
    ...
    if ! test -d "$(mason_packages)/$MASON_PLATFORM_ID/$1/$2"; then
        mason install $1 $2
...
MASON_LINKED_ABS=$(mason_packages)/.link
MASON_LINKED_REL=$(realpath -m --relative-to "$PWD" -- "$MASON_LINKED_ABS")
...
PATH_REPLACE = '$HOME/build/mapbox/mason/mason_packages:${MASON_LINKED_REL%/.link}'
